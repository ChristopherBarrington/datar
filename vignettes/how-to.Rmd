...
The default (`base`) `save` function is still accessible by including a `file` argument to the function.

## Loading data

The object can be loaded back into the environment using the `load` function. This function can also load  package's data in the expected way. By default, the function looks for the specified object(s) in `/project/path/data-repository/data` as well as the `data` subdirectory of all loaded packages.
...
There are two variables that can be customised: the path to the project and the path within the project to the data repository. Both can be defined in the `options()` list.
...
[BABS]: http://christopherbarrington.github.io
